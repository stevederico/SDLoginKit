...
- (void)didTapSendReset{
    ...
    NSString *email = [[(SDPlaceholderCell*)[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]] textField] text];
    ...
    if ([email length]>0) {
        ...
        if ([PFUser requestPasswordResetForEmail:email]){
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Email Sent" message:@"A Password Reset Email Has Been Sent to Your Email Address." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
            [alertView show];
        } else {
              [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Error" message:@"No Account Was Found With That Email Address" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
            [alertView show];
        }
          ...
          [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Please Enter an Email Address" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alertView show];
    }
